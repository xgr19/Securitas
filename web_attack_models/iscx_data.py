...
class ISCXDataset(torch.utils.data.Dataset):
    def __init__(self, x, label, direction=None):
        self.x = x
        self.label = label
...
def load_epoch_data(flow_dict, split='train'):
    ...
    return np.array(x), np.array(direction), np.array(label)[:, np.newaxis]


def clean_data(data, direction, label):
    del_list = []
...
def collate_attack_model(insts):
    x, label = list(zip(*insts))
    return torch.LongTensor(np.array(x)), torch.LongTensor(np.array(label)).contiguous().view(-1)
